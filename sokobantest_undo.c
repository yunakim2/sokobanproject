...
void UndoMap()
{
	...
	for (int j = 0; j < Y; j++)
	{
		for (int k = 0; k < X; k++)
		{
			undo1[save_count][j][k] = stageData[userlevel][j][k];
		}
	}
	save_count++;
}
...
void Undo()
{
	...
	for (int i = 0; i < Y; i++)
	{
		for (int j = 0; j < X; j++)
		{
			stageData[userlevel][j][k] = undo1[userlevel][i][j]; // 현재 맵을 언두맵에서 저장한 맵으로 대체함
		}
	}
	// @의 좌표값을 저장함
	for (int i = 0; i < Y; i++)
	...
	map[levelY + dy][levelX + dx] = undo_x; // 플레이어 x좌표 조정
	map[levelY + dy] = undo_y;
}