...
char sokoData[6][30][30]; // sokoban file map 입력받기
...
_Bool rank_bool = 0; //rankingview 실행 유무 bool
_Bool return_bool = 0; // return 하기 위한 bool
int dx, dy; //플레이어 이동
...
int StageClear() {
   ...
   for (x = 0; x < 30; x++) {
      for (y = 0; y < 30; y++) {
         if (map[userlevel][y][x] != '$' && stageData[userlevel][y][x] == 'O') { // 만약 초기맵에 구멍이 있는데 맵이 상자가 아니면 0을 리턴
            return 0;
         }
      }
   }
