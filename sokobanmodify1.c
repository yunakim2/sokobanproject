...
void PlayerMoveAction()
{
	...
	if (map[userlevel][levelY - 1 + dy][levelX + dx] == 'O') // 'O'를 만난다면
	{
		...
		{
			map[userlevel][levelY - 1 + dy][levelX + dx] = 'O'; // 'O' 계속 유지
		}  //
		UndoMap();//
	}

	if (map[userlevel][levelY - 1 + 2 * dy][levelX + 2 * dx] == 'O' && map[userlevel][levelY - 1 + dy][levelX + dx] == '$') // 상자를 밀고 있고, 상자 앞에 'O'가 있다면
	{
